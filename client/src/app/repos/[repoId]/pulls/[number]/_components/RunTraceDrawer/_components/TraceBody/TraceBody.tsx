  ...
  return (
    <>
      ...
      <TraceSection icon="FileText" title={t("trace.promptAssembly")} defaultOpen={false}>
        ...
        {trace.prompt_assembly.skill_blocks && trace.prompt_assembly.skill_blocks.length > 0
          ? trace.prompt_assembly.skill_blocks.map((sb) => (
              <PromptBlock
                key={sb.id}
                label={t("trace.prompt.skillBlock", { name: sb.name, tokens: sb.tokens })}
                text={sb.body}
