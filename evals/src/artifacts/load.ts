...
/**
 * SKILL.md plus every reference `.md` — the full payload the harness would assemble. In this repo
 * a skill keeps its reference files as flat siblings of SKILL.md (e.g. `layers.md`,
 * `dependency-rule.md`); a `references/` subdirectory is also honoured for skills that nest them.
 */
export function skillContent(skillName: string): string {
  ...
  // Flat sibling reference files (this repo's layout). Skip SKILL.md itself and any subdirectory.
  for (const f of readdirSync(dir).sort()) {
    if (f === "SKILL.md" || !f.endsWith(".md")) continue;
    const p = join(dir, f);
    if (statSync(p).isFile()) parts.push(`\n\n## Reference: ${f}\n\n${readFileSync(p, "utf8")}`);
  }
  // Optional references/ subdirectory (upstream layout), if a skill uses one instead.
