...
# Slack incoming webhook — posts a one-line summary (repo, PR number, findings
# by severity) when a review run finishes. The URL IS the credential, so it is
# read through the SecretsProvider like the API keys above (secrets.json wins
# over env). Leave empty to disable notifications entirely.
SLACK_WEBHOOK_URL=
...
export { SlackWebhookNotifier, type Notifier, type RunFinishedNotice } from './notify/index.js';
...
import type { Notifier, RunFinishedNotice } from './notify/index.js';
...
// ---------- Mock Notifier ----------
/** Records the notices a run posted — NO network. Injected via ContainerOverrides.notifier. */
export class MockNotifier implements Notifier {
  public sent: RunFinishedNotice[] = [];
...
## `server/src/adapters/notify/index.ts`
...
## `server/src/adapters/notify/slack.ts`
...
import type { RunFinishedNotice } from '../../adapters/notify/index.js';
...
/**
 * Map a finished run onto the vendor-neutral `RunFinishedNotice` the Notifier
 * port consumes: repo slug, PR number, finding counts split by severity.
 *
 * Boundary mapping, exactly like `reviewToDto` — the run-executor holds rows and
 * engine findings, the port speaks domain terms, and this is the seam between
 * them. Nothing here knows the notice ends up in Slack.
 */
...
import { taskLine, runFinishedNotice } from './helpers.js';
...
export class ReviewRunExecutor {
  ...
  private async runOneAgent(
    ...
    try {
      ...
      // The run is finished and durable — now tell the outside world. Posting
      // AFTER persistence + bus completion means a slow/broken webhook can
      // never delay or corrupt the run itself.
      await this.notifyRunFinished(repo, pull, keptFindings, runLog);
  ...
  /**
   * Post the finished run's summary (repo, PR number, findings by severity) to
   * the configured Notifier — resolved from the container, so this method names
   * a PORT, never a webhook or an HTTP client.
   *
   * Best-effort, exactly like the repo-intel enrichments above: no notifier
   * configured → silent no-op; a failed post is surfaced in the Live Log and
   * never fails a review that already succeeded and persisted.
   */
  private async notifyRunFinished(
    ...
    try {
      ...
      if (!notifier) return;
      await notifier.runFinished(runFinishedNotice(repo, pull, findings));
      runLog.info('notify: run summary posted');
    } catch (err) {
      runLog.info(`notify: failed to post run summary — ${(err as Error).message}`);
    }
  }
...
import { type Notifier, SlackWebhookNotifier } from '../adapters/notify/index.js';
...
export interface ContainerOverrides {
  ...
  depgraph?: DepGraph;
  tokenizer?: Tokenizer;
  /** Outbound run notifications (Slack webhook in prod) — tests inject MockNotifier. */
  notifier?: Notifier;
}
...
export class Container {
  ...
  // `undefined` = not resolved yet; `null` = resolved, no webhook configured.
  ...
  /**
   * Outbound run notifications. The webhook URL is a credential, so it comes
   * from the SecretsProvider (`SLACK_WEBHOOK_URL`), like every other secret —
   * never from AppConfig/process.env at the call site.
   *
   * Returns `null` when no webhook is configured: notifications are an OPTIONAL
   * side-channel, so "not configured" is a normal state the caller skips, not a
   * ConfigError that would fail an otherwise-successful review run.
   */
  async notifier(): Promise<Notifier | null> {
    if (this.overrides.notifier) return this.overrides.notifier;
    if (this._notifier !== undefined) return this._notifier;
    const url = await this.secrets.get('SLACK_WEBHOOK_URL');
    this._notifier = url ? new SlackWebhookNotifier(url) : null;
