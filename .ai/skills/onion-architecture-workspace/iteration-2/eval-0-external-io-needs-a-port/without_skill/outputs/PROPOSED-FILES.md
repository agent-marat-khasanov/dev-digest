...
# Slack incoming webhook — a one-line summary is posted when a review run
# finishes (repo, PR number, findings by severity). Read through SecretsProvider
# like the API keys. Unset → no message is sent (the feature is simply off).
SLACK_WEBHOOK_URL=
...
/** Count the run's findings per severity (zeros included) — the run-finished notice. */
...
import { countBySeverity, taskLine } from './helpers.js';
...
export class ReviewRunExecutor {
  ...
  private async runOneAgent(
    ...
    try {
      ...
      // The run is now 'done' in the DB — announce it on the team channel. Best-
      // effort and placed BEFORE the trace is built so a webhook failure lands in
      // the persisted log too (see notifyRunFinished: it never throws).
      await this.notifyRunFinished(repo, pull, agent, keptFindings, runLog);
  ...
  /**
   * Post the "run finished" summary to the team channel through the Notifier
   * port (Slack incoming webhook in prod). Best-effort, exactly like the prompt
   * enrichments: no webhook configured → the port no-ops; a transport failure is
   * surfaced as a Live Log line and NEVER fails a run whose review is already
   * persisted.
   */
  private async notifyRunFinished(
    ...
    try {
      await this.container.notifier.reviewFinished({
        repo: repo.fullName,
...
import { type Notifier, SlackWebhookNotifier } from '../adapters/notify/slack.js';
...
export interface ContainerOverrides {
  ...
  depgraph?: DepGraph;
  tokenizer?: Tokenizer;
  /** Outbound run notifications (Slack webhook in prod). */
  notifier?: Notifier;
}
...
export class Container {
  ...
  /**
   * Outbound notifications (Slack incoming webhook). The URL is a secret, so it
   * is resolved through SecretsProvider on every send rather than captured at
   * construction — a webhook added at runtime works without a cache flush, and
   * an unset one makes every send a no-op (no webhook configured → feature off).
   */
  get notifier(): Notifier {
    if (this.overrides.notifier) return this.overrides.notifier;
    this._notifier ??= new SlackWebhookNotifier(() => this.secrets.get('SLACK_WEBHOOK_URL'));
